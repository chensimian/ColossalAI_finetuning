...
class PerformanceEvaluator:
    ...
    def on_fit_end(self) -> None:
        avg_duration = all_reduce_mean(self.timer.duration, self.coordinator.world_size) #整个训练过程平均持续时间
        avg_throughput = self.num_samples * self.dp_world_size / (avg_duration + 1e-12) #平均吞吐量（单位时间内处理的样本数）
        mp_world_size = self.coordinator.world_size // self.dp_world_size #并行模型的大小 （总的设备数量/数据并行的设备数量）
        avg_tflops_per_gpu = self.flop / 1e12 / (avg_duration + 1e-12) / mp_world_size #每个GPU平均的TFLOPS
